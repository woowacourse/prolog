...
Feature: 역량 학습로그 맵핑 기능
  ...
  Scenario: 스터디로그 작성시 역량 맵핑하기
    Given 2, 4 역량을 맵핑한 "더새로운 스터디로그" 스터디로그를 작성하고
    When "브라운"이 작성한 역량이 맵핑된 학습로그를 조회하면
    Then "언어", "TDD" 역량이 맵핑된 "더새로운 스터디로그" 학습로그가 조회된다

  Scenario: 스터디로그 수정시 역량 맵핑하기
    Given 2 역량 한개를 맵핑한 "[알고리즘] 알고리즘은 어려워요" 스터디로그를 작성하고
    When 2번째 스터디로그의 역량을 수정하면
    When "브라운"이 작성한 역량이 맵핑된 학습로그를 조회하면
    Then "TDD" 역량 하나가 맵핑된 "[알고리즘] 알고리즘은 어려워요" 학습로그가 조회된다
