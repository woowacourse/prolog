...
Feature: 포스트 관련 기능
  ...
  Scenario: 조건별 포스트 목록 페이지 조회하기 - 검색어
    Given 서로 다른 태그와 미션을 가진 포스트를 다수 생성하고
    When "학습log"을 검색하면
    Then 12개의 포스트 목록을 받는다

  Scenario: 조건별 포스트 목록 페이지 조회하기 - 검색어 + 태그
    Given 서로 다른 태그와 미션을 가진 포스트를 다수 생성하고
    When "학습log"을 검색하고 1번 태그의 포스트를 조회하면
    Then 7개의 포스트 목록을 받는다

  Scenario: 조건별 포스트 목록 페이지 조회하기 - 검색어 + 태그 + 미션1
    Given 서로 다른 태그와 미션을 가진 포스트를 다수 생성하고
