...
class PopularStudylogServiceTest {
    ...
    @DisplayName("인기 학습로그를 갱신을 테스트한다.")
    @Nested
    class UpdatePopularStudylogs {

        @DisplayName("2주안에 작성된 인기 학습로그가 페이지에 표시될 수 있는 최대치만큼 존재할 떄")
        @Test
        void enoughWhileTwoCycle() {
            ...
            final List<GroupMember> groupMembers = Arrays.asList(splitGroupMember, journeyGroupMember);
            final List<Studylog> studylogs = Arrays.asList(splitStudylog, journeyStudylog);
            final List<PopularStudylog> popularStudylogs = Arrays.asList(splitPopularStudylog, journeyPopularStudylog);
        ...
        @DisplayName("2주동안 작성된 인기 학습로그가 페이지에 출력할 만큼 존재하지 않을 때")
        @Test
    ...
    @DisplayName("인기 학습로그를 조회한다.")
    @Nested
    class findPopularStudylogs {

        @DisplayName("익명의 사용자 일 때")
        @Test
