...
public class StudylogTagsTest {
    ...
    void addTest() {
        ...
        StudylogTag 레벨1_스터디로그_태그 = new StudylogTag(3L, 웨지가_쓴_글, 레벨1_태그);

        List<StudylogTag> studylogTags = Arrays.asList(워니_스터디로그_태그, 레벨1_스터디로그_태그);

        // when
        this.studylogTags.add(studylogTags);

        // then
        assertThat(this.studylogTags.getValues()).containsExactlyInAnyOrder(this.워니_스터디로그_태그, 레벨1_스터디로그_태그);
    }
    ...
    void updateTest() {
        ...
        List<StudylogTag> studylogTags = Arrays.asList(포비_스터디로그_태그, 레벨1_스터디로그_태그);

        // when
        this.studylogTags.update(studylogTags);

        // then
        assertThat(this.studylogTags.getValues()).containsExactlyInAnyOrder(포비_스터디로그_태그, 레벨1_스터디로그_태그);
    }
}
