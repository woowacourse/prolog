...
class StudylogServiceTest {
    ...
    void insertStudylogsTest() {
        ...
        //when
        //then
        List<String> titles = Stream.concat(studylogsOfMember1.stream(), studylogsOfMember2.stream())
        ...
        List<String> members = Stream.concat(studylogsOfMember1.stream(), studylogsOfMember2.stream())
    ...
    @ParameterizedTest
    @MethodSource
    void findWithFilter(List<Long> levelIds, List<Long> missionIds, List<Long> tagIds, List<String> usernames, List<String> expectedStudylogTitles) {
        //given
        insertStudylogs(member1, studylog1, studylog2);
        insertStudylogs(member2, studylog3, studylog4);

        StudylogsResponse studylogResponsesWithFilter =
                studylogService
                        .findStudylogsWithFilter(levelIds, missionIds, tagIds, usernames, PageRequest.of(0, 10));

        List<String> titles = studylogResponsesWithFilter.getData().stream()
                .map(StudylogResponse::getTitle)
                .collect(toList());
    ...
    private static Stream<Arguments> findWithFilter() {
        return Stream.of(
                Arguments.of(emptyList(), emptyList(), asList(tag1.getId(), tag2.getId(), tag3.getId()), asList(), asList(
                        STUDYLOG1_TITLE, STUDYLOG2_TITLE, STUDYLOG3_TITLE)),
                Arguments.of(emptyList(), emptyList(), singletonList(tag2.getId()), asList(), asList(
                        STUDYLOG1_TITLE, STUDYLOG2_TITLE)),
                Arguments.of(emptyList(), emptyList(), singletonList(tag3.getId()), asList(), asList(
                        STUDYLOG2_TITLE, STUDYLOG3_TITLE)),
                Arguments.of(emptyList(), singletonList(1L), emptyList(), asList(), asList(
                        STUDYLOG1_TITLE, STUDYLOG2_TITLE)),
                Arguments.of(emptyList(), singletonList(2L), emptyList(), asList(), asList(
                        STUDYLOG3_TITLE, STUDYLOG4_TITLE)),
                Arguments.of(emptyList(), singletonList(1L), singletonList(tag1.getId()), asList(), singletonList(
                        STUDYLOG1_TITLE)),
                Arguments.of(singletonList(1L), singletonList(1L), asList(tag1.getId(), tag2.getId(), tag3.getId()), asList(), asList(
                        STUDYLOG1_TITLE, STUDYLOG2_TITLE)),
                Arguments.of(emptyList(), singletonList(1L), singletonList(tag2.getId()), asList(), asList(
                        STUDYLOG1_TITLE, STUDYLOG2_TITLE)),
                Arguments.of(emptyList(), asList(1L, 2L), singletonList(tag3.getId()), asList(), asList(
                        STUDYLOG2_TITLE, STUDYLOG3_TITLE)),
                Arguments.of(emptyList(), emptyList(), emptyList(), asList(), asList(
                        STUDYLOG1_TITLE, STUDYLOG2_TITLE, STUDYLOG3_TITLE, STUDYLOG4_TITLE))
        );
    }
    ...
    void findStudylogsOfTest() {
        ...
        assertThat(expectedResultOfMember1).containsExactly(studylog1.getTitle(), studylog2.getTitle());
    ...
    void deleteStudylogTest() {
        //given
        List<StudylogResponse> studylogRespons = insertStudylogs(member1, studylog1, studylog2, studylog3,
                studylog4);
