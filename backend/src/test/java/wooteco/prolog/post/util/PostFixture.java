...
public enum PostFixture {
    ...
    자동차_미션_정리("자동차 미션 정리", "부릉 부릉 자동차가 나가신다 부릉부릉", "자동차 미션", Tags.of(Arrays.asList("자동차", "전략 패턴", "부릉"))),
    로또_미션_정리("로또 미션 정리", "따르르릉 로또", "로또 미션", Tags.of(Arrays.asList("로또", "TDD", "랜덤")));
    ...
    public static class InjectHelper {
        ...
        void init() {
            allOf(PostFixture.class)
                    .forEach(container -> container.injectMissionService(missionService));
        }
    }
    ...
    PostFixture(String title, String content, String missionName, Tags tags) {
        this.title = title;
    ...
    public PostRequest asRequestWithTags(List<TagRequest> tagRequests) {
        final MissionResponse missionResponse = createMission(missionName);
    ...
    private MissionResponse createMission(String missionName) {
        return missionService.create(new MissionRequest(missionName));
    }
}
